...
`django-fsm-ticket` is a customizable workflow and ticketing framework, built on top of [`django-fsm-2`](https://github.com/django-commons/django-fsm-2).
...
`django-fsm-ticket` takes care of automatically generating a fully-working UI from your definitions of `Ticket` and `TicketUpdate` subclasses. It displays the proper forms and actions. You can fully customize the templates, models, and views if you wish.
...
Here, we are specifying the base url and telling `django-fsm-ticket` to send email notifications to users in charge of taking actions on tickets unless they opted-out from email notifications.

You may want to be able to configure `django-fsm-ticket` via the Django admin interface by using `django-constance`, as discussed in the tutorial.
